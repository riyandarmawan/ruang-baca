<x-dashboard.layout title="{{ $title }}">
    <div x-data="{ activatedTab: 'profile', showModalHapus: false }" class="m-4 ms-auto max-w-[40rem] rounded bg-white p-4 shadow">
        <ul class="mb-4 flex border-b border-slate-200">
            <li @click="activatedTab = 'profile'" :class="activatedTab === 'profile' ? 'tab-active' : 'tab-inactive'"
                class="cursor-pointer rounded-tl rounded-tr border border-b-0 px-4 py-2 font-medium">
                Profile</li>
            <li @click="activatedTab = 'ubah'" :class="activatedTab === 'ubah' ? 'tab-active' : 'tab-inactive'"
                class="cursor-pointer rounded-tl rounded-tr border border-b-0 px-4 py-2 font-medium">
                Ubah</li>
            <li @click="activatedTab = 'ubahPassword'"
                :class="activatedTab === 'ubahPassword' ? 'tab-active' : 'tab-inactive'"
                class="cursor-pointer rounded-tl rounded-tr border border-b-0 px-4 py-2 font-medium">
                Ubah Password</li>
            <li @click="activatedTab = 'hapus'" :class="activatedTab === 'hapus' ? 'tab-active' : 'tab-inactive'"
                class="cursor-pointer rounded-tl rounded-tr border border-b-0 px-4 py-2 font-medium">
                Hapus</li>
        </ul>

        <div x-show="activatedTab === 'profile'" class="grid grid-cols-4">
            <div class="col-span-4 mb-8">
                <img src="{{ asset('storage/images/users/' . Auth::user()->profile) }}" alt="{{ Auth::user()->name }}"
        ...
        <div x-cloak x-show="activatedTab === 'ubah'">
            <form action="/dashboard/user/ubah/{{ Auth::user()->username }}" method="POST">
                ...
                <div class="mb-4 grid grid-cols-4 items-center">
                    ...
                    @error('name')
                        <p class="col-span-3 col-start-2 mt-2.5 text-sm font-medium text-red-500">{{ $message }}</p>
                    @enderror
                </div>
                <div class="mb-4 grid grid-cols-4 items-center">
                ...
                <button
                    class="col-span-4 mt-2 w-full rounded bg-tersier px-4 py-2 text-center font-medium text-white shadow">Ubah</button>
            </form>
        </div>

        <div x-cloak x-show="activatedTab === 'ubahPassword'">
            <form action="/dashboard/user/ubah-password/{{ Auth::user()->username }}" method="POST">
                ...
                <button
                    class="col-span-4 mt-2 w-full rounded bg-tersier px-4 py-2 text-center font-medium text-white shadow">Ubah
                    Password</button>
            </form>
        </div>
        ...
        <div x-cloak x-show="showModalHapus"
            ...
            <!-- Modal Container -->
            <div class="w-96 scale-100 transform rounded-lg bg-white p-6 shadow-lg transition-transform duration-300">
                <!-- Modal Header -->
                ...
                <div class="mt-6 flex justify-end space-x-3">
                    <button @click="showModalHapus = false"
                        class="rounded bg-gray-200 px-4 py-2 text-gray-700 transition duration-200 hover:bg-gray-300">
                        Cancel
                    </button>
                    <form action="/dashboard/user/hapus/{{ Auth::user()->username }}" method="POST">
                        @csrf
                        <button
                            class="rounded bg-red-500 px-4 py-2 text-white transition duration-200 hover:bg-red-600">
                            Delete
                        </button>
                    </form>
                </div>
