            ...
            <div
                ...
                <div x-bind:class="active !== 'detail' ? '' : '!grid'" class="hidden gap-4 p-4">
                    <div class="grid grid-cols-4">
                        <p>NISN</p>
                        <p class="col-span-3"><span class="me-2">:</span> {{ $pengembalian->nisn }}</p>
                    </div>
                    <div class="grid grid-cols-4">
                        <p>Buku</p>
                    ...
                    <div class="grid grid-cols-4">
                        <p>Tanggal Kembali</p>
                        <p class="col-span-3"><span class="me-2">:</span> {{ $pengembalian->tanggal_kembali }}</p>
                    </div>
                </div>

                <div x-bind:class="active !== 'ubah' ? '' : '!block'" class="hidden p-4">
                    <form action="/dashboard/pengembalian/detail/{{ $pengembalian->id }}" method="POST">
                        <!-- Siswa Selection -->
                        <div class="mb-4 grid grid-cols-3 items-center">
                            ...
                            <select name="nisn" id="nisn"
                                ...
                                @foreach ($siswas as $siswa)
                                    <option {{ $pengembalian->nisn === $siswa->nisn ? 'selected' : '' }}
                ...
                <div x-bind:class="active !== 'hapus' ? '' : '!block'" class="hidden p-4">
                    <a x-on:click="modal = !modal" x-ref="delete" href="/dashboard/siswa/hapus/{{ $pengembalian->id }}"
                        @click.prevent="" class="font-medium text-red-500">Hapus data pengembalian ini</a>
                </div>
            </div>
        </div>
